...
class ScoreData:
    """ArrivalObserver that accumulates each color's score by the point
    value of every piece it captures - registered directly with
    GameEngine, exactly like PieceAnimatorRegistry. Scoring reacts to
    arrivals only, so on_arrival is the single hook it declares.
    Captures are read straight off ArrivalEvent.captured_piece, the same
    field GameEngine itself already keys off for the king-capture win
    condition; there is no separate "on_capture" notification."""
    ...
    def on_arrival(self, event: ArrivalEvent) -> None:
        """Credit the arriving piece's color with the captured piece's
        point value, if any."""
